...
pub enum DownsampleStrategy {
    ...
    Average,
    /// Skip bin aggregation entirely and rely on raw waveform data.
    Disabled,
}

impl Default for DownsampleStrategy {
    fn default() -> Self {
...
pub struct WaveformProcessor {
    ...
    prefilter_prev1: Vec<f32>,
    prefilter_prev2: Vec<f32>,
    prefilter_initialized: Vec<bool>,
    track_average: bool,
...
impl WaveformProcessor {
    pub fn new(config: WaveformConfig) -> Self {
        ...
        let mut processor = Self {
            ...
            prefilter_prev1: Vec::new(),
            prefilter_prev2: Vec::new(),
            prefilter_initialized: Vec::new(),
    ...
    fn resize_columns(&mut self) {
        ...
        ensure_len(&mut self.prefilter_prev1, channels, 0.0);
        ensure_len(&mut self.prefilter_prev2, channels, 0.0);
        self.prefilter_initialized.resize(channels, false);
    ...
    fn adjust_capacity(&mut self, old_capacity: usize, new_capacity: usize) {
        ...
        if keep > 0 {
            ...
            for channel in 0..channels {
                ...
                let dest_base = channel * new_capacity;
                for i in 0..keep {
                    let src_idx = if continuous {
                        start_index + i
                    } else {
                        (start_index + i) % old_capacity
                    };
                    new_min[dest_base + i] = self.min_columns[src_base + src_idx];
                    new_max[dest_base + i] = self.max_columns[src_base + src_idx];
                }
            }
            ...
            for i in 0..keep {
                ...
                new_freq[i] = self.freq_columns[src_idx];
            }
        }
    ...
    fn flush_current_bucket(&mut self) {
        ...
        for channel in 0..channels {
            ...
            if self.track_average {
                ...
                self.min_columns[offset] = -amplitude;
                self.max_columns[offset] = amplitude;
            } else {
                let min_val = if self.current_min[channel] == f32::MAX {
                    0.0
                } else {
                    self.current_min[channel] - mean
                };
                let max_val = if self.current_max[channel] == f32::MIN {
                    0.0
                } else {
                    self.current_max[channel] - mean
                };
    ...
    fn prefilter_sample(&mut self, channel: usize, sample: f32) -> f32 {
        if self.prefilter_prev1.len() <= channel {
            self.prefilter_prev1.resize(channel + 1, 0.0);
        }
        if self.prefilter_prev2.len() <= channel {
            self.prefilter_prev2.resize(channel + 1, 0.0);
        }
        if self.prefilter_initialized.len() <= channel {
            self.prefilter_initialized.resize(channel + 1, false);
        }

        let initialized = self.prefilter_initialized[channel];
        let prev1 = if initialized {
            self.prefilter_prev1[channel]
        } else {
            sample
        };
        let prev2 = if initialized {
            self.prefilter_prev2[channel]
        } else {
            sample
        };

        let filtered = prev2 * 0.25 + prev1 * 0.5 + sample * 0.25;
        self.prefilter_prev2[channel] = prev1;
        self.prefilter_prev1[channel] = sample;
        self.prefilter_initialized[channel] = true;
        ...
        for frame in samples.chunks_exact(channels) {
            ...
            for channel in 0..channels {
                if let Some(history) = self.raw_history.get_mut(channel) {
                    history.push(frame[channel]);
                }
            }

            let mut filtered = self.prefilter_sample(0, frame[0]);
            self.current_min[0] = self.current_min[0].min(filtered);
            self.current_max[0] = self.current_max[0].max(filtered);
            self.current_sum[0] += filtered;
            self.current_sum_squares[0] += filtered * filtered;
            ...
            for channel in 1..channels {
                filtered = self.prefilter_sample(channel, frame[channel]);
                self.current_min[channel] = self.current_min[channel].min(filtered);
                self.current_max[channel] = self.current_max[channel].max(filtered);
                self.current_sum[channel] += filtered;
                self.current_sum_squares[channel] += filtered * filtered;
            }
    ...
    fn populate_snapshot(&mut self) {
        ...
        resize_vec(&mut self.snapshot.min_values, columns * channels);
        resize_vec(&mut self.snapshot.max_values, columns * channels);
        resize_vec(&mut self.snapshot.frequency_normalized, columns);

        if columns == 0 {
            self.snapshot.min_values.fill(0.0);
            self.snapshot.max_values.fill(0.0);
            self.snapshot.frequency_normalized.fill(0.0);
        ...
        for channel in 0..channels {
            ...
            let dest_base = channel * columns;
            for i in 0..columns {
                let src = (start + i) % capacity;
                let dest = dest_base + i;
                self.snapshot.min_values[dest] = self.min_columns[src_base + src];
                self.snapshot.max_values[dest] = self.max_columns[src_base + src];
            }
        }

        for i in 0..columns {
            let src = (start + i) % capacity;
            self.snapshot.frequency_normalized[i] = self.freq_columns[src];
        }
        ...
        if raw_frame_count == 0 {
            self.snapshot.raw.samples.clear();
        } else {
            let required = raw_frame_count * channels;
            if self.snapshot.raw.samples.len() < required {
                self.snapshot.raw.samples.resize(required, 0.0);
            } else {
                self.snapshot.raw.samples[..required].fill(0.0);
                self.snapshot.raw.samples.truncate(required);
            }
            ...
            for (channel_index, history) in self.raw_history.iter().enumerate() {
                ...
                history.write_interleaved(&mut self.snapshot.raw.samples, channels, channel_index);
            }
        }
    ...
    fn update_preview_snapshot(&mut self) {
        ...
        let frames = self.current_samples as f32;
        resize_vec(&mut self.snapshot.preview.min_values, channels);
        resize_vec(&mut self.snapshot.preview.max_values, channels);
        ...
        for channel in 0..channels {
            ...
            self.snapshot.preview.min_values[channel] = min_value;
            self.snapshot.preview.max_values[channel] = max_value;
        }
...
impl AudioProcessor for WaveformProcessor {
    ...
    fn reset(&mut self) {
        ...
        self.bucket_prev_time = 0.0;
        self.bucket_elapsed_seconds = 0.0;
        self.prefilter_prev1.clear();
        self.prefilter_prev2.clear();
        self.prefilter_initialized.clear();
...
mod tests {
    ...
    fn samples_for_columns(config: &WaveformConfig, columns: usize, channels: usize) -> Vec<f32> {
        let frames_per_column = nominal_frames_per_column(config).ceil() as usize;
        let total_frames = frames_per_column * columns;
        let mut samples = Vec::with_capacity(total_frames * channels);
        for i in 0..(total_frames * channels) {
            samples.push(((i % 128) as f32 / 128.0) * 2.0 - 1.0);
        }
        samples
    }

    #[test]
    fn respects_column_capacity() {
        ...
        let expected_capacity = config.max_columns;
        let mut processor = WaveformProcessor::new(config);

        let channels = 2;
        let samples = samples_for_columns(&processor.config, expected_capacity * 2, channels);
        ...
        assert_eq!(snapshot.columns, expected_capacity);
        let expected_spacing = 1.0 / processor.config.scroll_speed;
        assert!((snapshot.column_spacing_seconds - expected_spacing).abs() < 1e-6);
    }
    ...
    fn downsampling_produces_min_max_pairs() {
        ...
        assert!(max_value > 0.3, "expected to preserve positive peaks");
        assert!(min_value <= 0.0, "expected min to remain non-positive");
        assert!((span - 0.375).abs() < 1e-3);
    ...
    fn average_downsampling_produces_envelope() {
        ...
        let max_value = snapshot.max_values[0];
        let min_value = snapshot.min_values[0];
        assert!(max_value > 0.0, "expected positive average amplitude");
        assert!(min_value < 0.0, "expected negative average amplitude");
        assert!(
            (max_value + min_value).abs() < 1e-6,
            "expected symmetric envelope"
        );
    ...
    #[test]
    fn snapshot_reflects_disabled_downsampling() {
        let mut processor = WaveformProcessor::new(WaveformConfig {
            sample_rate: DEFAULT_SAMPLE_RATE,
            scroll_speed: DEFAULT_SCROLL_SPEED,
            downsample: DownsampleStrategy::Disabled,
            max_columns: DEFAULT_COLUMN_CAPACITY,
        });

        let samples = vec![0.0; 512];
        let block = make_block(&samples, 2, DEFAULT_SAMPLE_RATE);
        processor.process_block(&block);

        assert_eq!(
            processor.snapshot().downsample,
