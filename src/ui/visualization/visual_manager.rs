    ...
    pub fn apply_visual_settings(&mut self, settings: &VisualSettings) {
        for entry in &mut self.entries {
            if let Some(module) = settings.modules.get(&entry.slot.kind) {
                ...
                match entry.slot.kind {
                    VisualKind::Oscilloscope => {
                        if let (Some(stored), Some(config)) =
                            (module.oscilloscope(), entry.runtime.oscilloscope_config())
                        {
                    ...
                    VisualKind::Spectrum => {
                        if let (Some(stored), Some(config)) =
                            (module.spectrum(), entry.runtime.spectrum_config())
                        {
                            let mut updated = config;
                            stored.apply_to(&mut updated);
                    ...
                    VisualKind::Spectrogram => {
                        if let (Some(stored), Some(config)) =
                            (module.spectrogram(), entry.runtime.spectrogram_config())
                        {
                            let mut updated = config;
                            stored.apply_to(&mut updated);
